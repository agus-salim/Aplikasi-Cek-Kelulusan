...
function admin_is_header_row(string $value): bool
{
    ...
    $headers = ['nisn', 'nama siswa', 'nama', 'status', 'cetak skl', 'no', 'number', 'template', 'keterangan'];
...
/**
 * Daftar nomor halaman dengan jendela di sekitar halaman aktif.
 * Mengembalikan array berisi int (nomor halaman) atau string '...' (jeda).
 */
function admin_page_list(int $current, int $total): array
{
    if ($total <= 7) {
...
/**
 * URL navigasi pagination tabel siswa (relatif, mempertahankan tab & per_page).
 */
function admin_siswa_url(int $page, string $perPageRaw): string
{
    return '?tab=section-data-siswa&page=' . $page . '&per_page=' . urlencode($perPageRaw);
}
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <button type="button"
                        ...
                        data-target="login-password" aria-label="Lihat password" title="Lihat Password">
...
if (isset($_GET['hapus_nisn']) && trim((string) $_GET['hapus_nisn']) !== '') {
    ...
    // Pertahankan halaman & jumlah data per halaman setelah hapus
    $backPage = max(1, (int) ($_GET['page'] ?? 1));
    $backPerPage = (string) ($_GET['per_page'] ?? '50');
    redirect('admin.php?success=deleted&tab=section-data-siswa&page=' . $backPage . '&per_page=' . urlencode($backPerPage));
    exit;
}
        ...
        switch ($action) {
            ...
            case 'tambah_siswa':
                ...
                if ($nisn === '' || $nama === '') {
                    ...
                    $stmt = $conn->prepare("INSERT INTO siswa (nisn, nama, status_kelulusan, cetak_skl) VALUES (?, ?, ?, 1)");
                    if ($stmt) {
                        $stmt->bind_param('ssi', $nisn, $nama, $status);
                        if ($stmt->execute()) {
                            $success = 'Siswa berhasil ditambahkan!';
                        } else {
                            $error = ($conn->errno === 1062) ? 'NISN sudah terdaftar!' : 'Gagal menambahkan siswa.';
                        }
                        $stmt->close();
                    } else {
                ...
                if ($usernameBaru === '' || $passwordBaru === '' || $konfirmasiPassword === '') {
                    ...
                    if ($stmt) {
                        ...
                        if ($stmt->execute()) {
                            ...
                            $error = ($conn->errno === 1062) ? 'Username sudah digunakan!' : 'Gagal menambahkan pengguna.';
                        }
                        $stmt->close();
                    } else {
                ...
                if ($userId <= 0) {
                    ...
                    if ($stmt) {
                        if ($stmt->execute()) {
                            ...
                            $error = ($conn->errno === 1062) ? 'Username sudah digunakan!' : 'Gagal memperbarui pengguna.';
                        }
                        $stmt->close();
                    } else {
            ...
            case 'delete_selected':
                if (isset($_POST['selected_nisn']) && is_array($_POST['selected_nisn'])) {
                    $list = array_values(array_filter(array_map('trim', array_map('strval', $_POST['selected_nisn']))));
                    if (empty($list)) {
            ...
            case 'import':
                ...
                if ($ext === 'csv') {
                    ...
                    while (($row = fgetcsv($handle, 0, ',')) !== false) {
                        ...
                        if ($status === '') { $status = '1'; }
                        if ($cetak === '') { $cetak = '1'; }
                        if (is_numeric($nisn)) { $nisn = (string) (int) $nisn; }
                        if ($nisn === '' || admin_is_header_row($nisn) || !ctype_digit($nisn)) { $skipped++; continue; }
                        if (admin_process_student_row($conn, $nisn, $nama, $status, $cetak)) { $imported++; } else { $failed++; }
                    }
                    fclose($handle);
...
$dataSiswa = [];
try {
    $result = $conn->query("SELECT nisn, nama, status_kelulusan, IFNULL(cetak_skl, 1) AS cetak_skl FROM siswa ORDER BY nama ASC");
    if ($result) {
...
$perPageRaw = (string) ($_GET['per_page'] ?? $_POST['per_page'] ?? '50');
if ($perPageRaw === 'all') {
    $perPage = null;
} elseif (in_array((int) $perPageRaw, $perPageOptions, true)) {
...
$totalPages = $perPage === null ? 1 : max(1, (int) ceil($totalSiswa / $perPage));
$currentPage = max(1, (int) ($_GET['page'] ?? $_POST['page'] ?? 1));
...
if ($perPage === null) {
    $offset = 0;
    $dataSiswaPage = $dataSiswa;
} else {
    $offset = ($currentPage - 1) * $perPage;
    $dataSiswaPage = array_slice($dataSiswa, $offset, $perPage);
}

$rangeStart = $totalSiswa === 0 ? 0 : $offset + 1;
$rangeEnd = min($totalSiswa, $offset + count($dataSiswaPage));
$pageList = admin_page_list($currentPage, $totalPages);
...
<aside class="w-64 bg-white border-r border-gray-200 hidden md:flex flex-col sticky top-0 h-screen z-20">
    ...
    <nav class="flex-1 overflow-y-auto p-4 space-y-1" aria-label="Menu admin">
        ...
        <a href="#" class="js-nav-link flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 transition-all" data-target="<?= admin_escape($menu['id']) ?>">
    ...
    <div class="p-4 border-t border-gray-100 space-y-2">
        <a href="index.php" target="_blank" rel="noopener noreferrer" class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 transition-all w-full">
            <i class="fa-solid fa-globe w-5 text-center" aria-hidden="true"></i>
            Lihat Situs
        </a>
        <a href="?logout=1" class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-red-600 hover:bg-red-50 transition-all w-full">
...
<main class="flex-1 p-4 md:p-8 overflow-y-auto relative">
    <div class="md:hidden flex items-center justify-between mb-6 bg-white p-4 rounded-xl shadow-sm sticky top-0 z-30">
        <button id="mobile-menu-btn" class="text-gray-600 hover:text-emerald-700 focus:outline-none p-1 rounded-lg hover:bg-gray-100 transition" aria-label="Buka menu navigasi">
        ...
        <div class="flex items-center gap-1">
            <a href="index.php" target="_blank" rel="noopener noreferrer" class="text-gray-600 hover:text-emerald-600 p-2 rounded-lg hover:bg-gray-100 transition" aria-label="Lihat situs">
                <i class="fa-solid fa-globe text-lg" aria-hidden="true"></i>
            </a>
            <a href="?logout=1" class="text-red-600 hover:bg-red-50 p-2 rounded-lg transition" aria-label="Logout">
    ...
    <?php if ($success !== ''): ?>
    <div id="auto-dismiss-alert" class="bg-emerald-50 border border-emerald-200 text-emerald-800 px-4 py-3 rounded-xl mb-6 flex items-center gap-3 shadow-sm transition-all duration-500 ease-in-out max-h-40 opacity-100" role="status">
        <i class="fa-solid fa-circle-check text-emerald-600" aria-hidden="true"></i>
        <span><?= admin_escape($success) ?></span>
    </div>
    <?php elseif ($error !== ''): ?>
    <div id="auto-dismiss-alert" class="bg-red-50 border border-red-200 text-red-800 px-4 py-3 rounded-xl mb-6 flex items-center gap-3 shadow-sm transition-all duration-500 ease-in-out max-h-40 opacity-100" role="alert">
    ...
    <div id="section-beranda" class="content-section space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
                <div class="w-14 h-14 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center text-2xl"><i class="fa-solid fa-users" aria-hidden="true"></i></div>
                <div><p class="text-sm text-gray-500 font-medium">Total Siswa</p><p class="text-3xl font-bold text-gray-800"><?= (int) $totalSiswa ?></p></div>
            </div>
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
                <div class="w-14 h-14 bg-emerald-100 text-emerald-600 rounded-xl flex items-center justify-center text-2xl"><i class="fa-solid fa-graduation-cap" aria-hidden="true"></i></div>
                <div><p class="text-sm text-gray-500 font-medium">Lulus</p><p class="text-3xl font-bold text-gray-800"><?= (int) $lulus ?></p></div>
            </div>
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
                <div class="w-14 h-14 bg-rose-100 text-rose-600 rounded-xl flex items-center justify-center text-2xl"><i class="fa-solid fa-book-open" aria-hidden="true"></i></div>
                <div><p class="text-sm text-gray-500 font-medium">Tidak Lulus</p><p class="text-3xl font-bold text-gray-800"><?= (int) $tidakLulus ?></p></div>
            </div>
        </div>
    </div>
            ...
            <form method="post" enctype="multipart/form-data" class="space-y-6" autocomplete="off">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" id="nama_sekolah" name="nama_sekolah" value="<?= admin_escape($namaSekolah) ?>" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                    </div>
                    <div>
                        <label for="tahun_pelajaran" class="block text-sm font-medium text-gray-700 mb-2">Tahun Pelajaran</label>
                        <input type="text" id="tahun_pelajaran" name="tahun_pelajaran" value="<?= admin_escape($tahunPelajaran) ?>" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                    </div>
                </div>
                <div>
                    <label for="logo_sekolah" class="block text-sm font-medium text-gray-700 mb-2">Logo Sekolah</label>
                    <input type="file" id="logo_sekolah" name="logo_sekolah" accept=".jpg,.jpeg,.png,.webp" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 transition">
                    <?php if ($logoSekolah !== ''): ?>
                    <div class="mt-3 flex items-center gap-3">
                        <img src="<?= admin_escape($logoSekolah) ?>" class="h-16 w-16 object-contain border rounded-lg p-1" alt="Logo sekolah" loading="lazy">
                        <span class="text-sm text-gray-500"><?= admin_escape(basename($logoSekolah)) ?></span>
                    </div>
                    <?php endif; ?>
                </div>
                <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-6 py-3 rounded-xl transition shadow-md shadow-emerald-200 flex items-center gap-2">
            ...
            <form method="post" autocomplete="off">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" id="nomor_wa" name="nomor_wa" value="<?= admin_escape($nomorWaTampil) ?>" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Preview Link</label>
                        <div class="bg-green-50 border border-green-200 rounded-xl p-4">
                            <a href="https://wa.me/<?= admin_escape($nomorWaLink) ?>" target="_blank" rel="noopener noreferrer" class="text-green-700 font-medium hover:underline break-all flex items-center gap-2">
                ...
                <button type="submit" class="mt-6 bg-green-600 hover:bg-green-700 text-white font-semibold px-6 py-3 rounded-xl transition shadow-md shadow-green-200 flex items-center gap-2">
                    <i class="fa-solid fa-save" aria-hidden="true"></i>
                    Simpan WhatsApp
                </button>
            </form>
            ...
            <form method="post" enctype="multipart/form-data" class="space-y-6" autocomplete="off">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" id="nama_kabupaten" name="nama_kabupaten" value="<?= admin_escape($sklNamaKabupaten) ?>" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                    </div>
                    <div>
                        <label for="tanggal_surat" class="block text-sm font-medium text-gray-700 mb-2">Tanggal Surat</label>
                        <input type="date" id="tanggal_surat" name="tanggal_surat" value="<?= admin_escape($sklTanggal) ?>" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                    </div>
                </div>
                <div>
                    <label for="nomor_surat" class="block text-sm font-medium text-gray-700 mb-2">Nomor Surat</label>
                    <input type="text" id="nomor_surat" name="nomor_surat" value="<?= admin_escape($sklNomor) ?>" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                    <p class="text-xs text-gray-500 mt-1">Gunakan <code class="bg-gray-200 px-1 rounded">{status}</code> untuk LULUS/TIDAK LULUS otomatis.</p>
                </div>
                <div>
                    <label for="isi_surat" class="block text-sm font-medium text-gray-700 mb-2">Isi Surat</label>
                    <textarea id="isi_surat" name="isi_surat" rows="5" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition"><?= admin_escape($sklIsi) ?></textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="text" id="nama_kepala" name="nama_kepala" value="<?= admin_escape($sklNamaKepala) ?>" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                    </div>
                    <div>
                        <label for="jabatan_kepala" class="block text-sm font-medium text-gray-700 mb-2">Jabatan</label>
                        <input type="text" id="jabatan_kepala" name="jabatan_kepala" value="<?= admin_escape($sklJabatan) ?>" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                    </div>
                    <div>
                        <label for="nip_kepala" class="block text-sm font-medium text-gray-700 mb-2">NIP</label>
                        <input type="text" id="nip_kepala" name="nip_kepala" value="<?= admin_escape($sklNip) ?>" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="file" id="kop_surat" name="kop_surat" accept=".jpg,.jpeg,.png" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 transition">
                        <?php if ($sklKop !== ''): ?>
                        <div class="mt-3"><img src="<?= admin_escape($sklKop) ?>" class="h-16 border rounded p-1" alt="Kop surat" loading="lazy"></div>
                        <?php endif; ?>
                    </div>
                    <div>
                        <label for="ttd_kepala" class="block text-sm font-medium text-gray-700 mb-2">Scan TTD + Cap</label>
                        <input type="file" id="ttd_kepala" name="ttd_kepala" accept=".jpg,.jpeg,.png" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 transition">
                        <?php if ($sklTtd !== ''): ?>
                        <div class="mt-3"><img src="<?= admin_escape($sklTtd) ?>" class="h-16 border rounded p-1" alt="Tanda tangan dan cap" loading="lazy"></div>
                        <?php endif; ?>
                    </div>
                </div>
                <div class="bg-gray-50 p-6 rounded-xl border border-gray-200">
                    ...
                    <div class="flex flex-wrap gap-4">
                        <label class="flex-1 min-w-[200px] cursor-pointer p-4 rounded-xl border-2 <?= $isSklAktif ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 bg-white' ?> transition-all flex items-center gap-3">
                            ...
                            <div><p class="font-bold text-emerald-800">Aktifkan SKL</p><p class="text-xs text-gray-600">Tombol Cetak & PDF muncul</p></div>
                        </label>
                        <label class="flex-1 min-w-[200px] cursor-pointer p-4 rounded-xl border-2 <?= !$isSklAktif ? 'border-red-500 bg-red-50' : 'border-gray-200 bg-white' ?> transition-all flex items-center gap-3">
                            ...
                            <div><p class="font-bold text-red-800">Nonaktifkan SKL</p><p class="text-xs text-gray-600">Tombol disembunyikan</p></div>
                        </label>
                    </div>
                </div>
                <button type="submit" class="mt-6 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-6 py-3 rounded-xl transition shadow-md shadow-emerald-200 flex items-center gap-2">
                    <i class="fa-solid fa-save" aria-hidden="true"></i>
                    Simpan SKL
                </button>
            </form>
            ...
            <form method="post" autocomplete="off">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="datetime-local" id="waktu_mulai" name="waktu_mulai" value="<?= admin_escape($waktuMulaiFormValue) ?>" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                    </div>
                    <div>
                        ...
                        <div class="bg-emerald-50 border border-emerald-200 text-emerald-800 px-4 py-3 rounded-xl font-semibold flex items-center gap-2">
                            <i class="fa-solid fa-unlock" aria-hidden="true"></i> SUDAH DIBUKA
                        </div>
                        ...
                        <div class="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 rounded-xl font-semibold flex items-center gap-2">
                            <i class="fa-solid fa-lock" aria-hidden="true"></i> BELUM DIBUKA
                        </div>
                        <?php else: ?>
                        <div class="bg-gray-50 border border-gray-200 text-gray-600 px-4 py-3 rounded-xl font-semibold flex items-center gap-2">
                            <i class="fa-solid fa-circle-question" aria-hidden="true"></i> BELUM DIATUR
                        </div>
                        <?php endif; ?>
                    </div>
                </div>
    ...
    <div id="section-data-siswa" class="content-section hidden space-y-6">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
            ...
            <form method="post" autocomplete="off">
                ...
                <input type="hidden" name="per_page" value="<?= admin_escape($perPageRaw) ?>">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        <label for="nisn" class="block text-sm font-medium text-gray-700 mb-2">NISN</label>
                        <input type="text" id="nisn" name="nisn" required inputmode="numeric" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                    </div>
                    <div>
                        <label for="nama" class="block text-sm font-medium text-gray-700 mb-2">Nama</label>
                        <input type="text" id="nama" name="nama" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                    </div>
                    <div>
                        <label for="status" class="block text-sm font-medium text-gray-700 mb-2">Status</label>
                        <select id="status" name="status" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
            ...
            <div class="mb-4">
                <a href="?download_template=1" class="inline-flex items-center gap-2 text-emerald-700 bg-emerald-50 hover:bg-emerald-100 px-4 py-2 rounded-lg font-medium transition">
            ...
            <form method="post" enctype="multipart/form-data" autocomplete="off">
                ...
                <div class="flex items-center gap-4">
                    <input type="file" name="file_excel" accept=".xlsx,.xls,.csv" required class="flex-1 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 transition">
                    <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-6 py-3 rounded-xl transition shadow-md shadow-emerald-200 flex items-center gap-2">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 overflow-hidden">
            <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
                ...
                <div class="flex flex-wrap gap-2">
                    <button type="button" id="btn-delete-selected" class="bg-red-50 hover:bg-red-100 text-red-700 px-4 py-2 rounded-lg text-sm font-semibold transition flex items-center gap-2">
                        <i class="fa-solid fa-trash" aria-hidden="true"></i>
                        Hapus Centang
                    </button>
                    <button type="button" id="btn-delete-all" data-total="<?= (int) $totalSiswa ?>" class="bg-rose-600 hover:bg-rose-700 text-white px-4 py-2 rounded-lg text-sm font-semibold transition flex items-center gap-2">
                        <i class="fa-solid fa-triangle-exclamation" aria-hidden="true"></i>
                        Hapus SEMUA
                    </button>
                    <button type="button" id="btn-save-skl" class="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-semibold transition flex items-center gap-2">
                        <i class="fa-solid fa-save" aria-hidden="true"></i>
                        Simpan Cetak SKL
                    </button>
                </div>
            ...
            <!-- Kontrol "Tampilkan" + info rentang data -->
            <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
                <label for="per-page-select" class="text-sm text-gray-600 flex items-center gap-2">
                    Tampilkan
                    <select id="per-page-select" class="px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm font-medium focus:ring-2 focus:ring-emerald-500 outline-none transition cursor-pointer">
                ...
                <p class="text-sm text-gray-600">
                    Menampilkan <strong class="text-gray-800"><?= (int) $rangeStart ?>–<?= (int) $rangeEnd ?></strong> dari <strong class="text-gray-800"><?= (int) $totalSiswa ?></strong> data
                </p>
            </div>
                ...
                <div class="overflow-x-auto rounded-xl border border-gray-200">
                    <table class="w-full text-sm text-left">
                        <thead class="bg-gray-50 text-gray-700 font-semibold border-b border-gray-200">
                            <tr>
                                <th class="p-4 w-10 text-center">
                                    <input type="checkbox" id="select-all" class="w-4 h-4 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500" aria-label="Pilih semua data">
                                </th>
                                <th class="p-4">No</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <?php foreach ($dataSiswaPage as $index => $siswa): ?>
                            <?php
                            $nisn = (string) ($siswa['nisn'] ?? '');
                            $nama = (string) ($siswa['nama'] ?? '');
                            $statusLulus = (int) ($siswa['status_kelulusan'] ?? 0) === 1;
                            $cetakSkl = (int) ($siswa['cetak_skl'] ?? 1) === 1;
                            $nomorUrut = $offset + $index + 1;
                            ?>
                            <tr class="hover:bg-gray-50 transition">
                                <td class="p-4 text-center">
                                    <input type="checkbox" name="selected_nisn[]" value="<?= admin_escape($nisn) ?>" class="row-checkbox w-4 h-4 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500" aria-label="Pilih siswa <?= admin_escape($nama) ?>">
                                    <input type="hidden" name="all_nisn[]" value="<?= admin_escape($nisn) ?>">
                                </td>
                                <td class="p-4 text-gray-500"><?= (int) $nomorUrut ?></td>
                                <td class="p-4 font-medium text-gray-800"><?= admin_escape($nisn) ?></td>
                                <td class="p-4"><?= admin_escape($nama) ?></td>
                                <td class="p-4">
                                ...
                                <td class="p-4 text-center">
                                    <input type="checkbox" name="cetak_skl[<?= admin_escape($nisn) ?>]" value="1" <?= $cetakSkl ? 'checked' : '' ?> class="w-5 h-5 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500 cursor-pointer" aria-label="Cetak SKL untuk <?= admin_escape($nama) ?>">
                                </td>
                                <td class="p-4 text-center">
                                    <a href="?hapus_nisn=<?= urlencode($nisn) ?>&page=<?= (int) $currentPage ?>&per_page=<?= urlencode($perPageRaw) ?>"
            ...
            <!-- Pagination -->
            <?php if ($totalPages > 1): ?>
            <nav class="mt-6 flex flex-wrap items-center justify-center gap-1" aria-label="Navigasi halaman">
                <?php if ($currentPage > 1): ?>
                <a href="<?= admin_siswa_url(1, $perPageRaw) ?>" class="px-3 py-2 rounded-lg text-sm text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 transition" title="Halaman pertama">«</a>
                <a href="<?= admin_siswa_url($currentPage - 1, $perPageRaw) ?>" class="px-3 py-2 rounded-lg text-sm text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 transition" title="Sebelumnya">‹</a>
                <?php else: ?>
                <span class="px-3 py-2 rounded-lg text-sm text-gray-300 cursor-not-allowed">«</span>
                <span class="px-3 py-2 rounded-lg text-sm text-gray-300 cursor-not-allowed">‹</span>
                <?php endif; ?>
                ...
                <?php foreach ($pageList as $item): ?>
                    ...
                    <span class="px-3.5 py-2 rounded-lg text-sm font-bold bg-emerald-600 text-white shadow-sm"><?= (int) $item ?></span>
                    <?php else: ?>
                    <a href="<?= admin_siswa_url((int) $item, $perPageRaw) ?>" class="px-3.5 py-2 rounded-lg text-sm text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 transition"><?= (int) $item ?></a>
                    <?php endif; ?>
                <?php endforeach; ?>

                <?php if ($currentPage < $totalPages): ?>
                <a href="<?= admin_siswa_url($currentPage + 1, $perPageRaw) ?>" class="px-3 py-2 rounded-lg text-sm text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 transition" title="Selanjutnya">›</a>
                <a href="<?= admin_siswa_url($totalPages, $perPageRaw) ?>" class="px-3 py-2 rounded-lg text-sm text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 transition" title="Halaman terakhir">»</a>
                <?php else: ?>
                <span class="px-3 py-2 rounded-lg text-sm text-gray-300 cursor-not-allowed">›</span>
                <span class="px-3 py-2 rounded-lg text-sm text-gray-300 cursor-not-allowed">»</span>
                <?php endif; ?>
    ...
    <div id="section-pengguna" class="content-section hidden space-y-6">
            ...
            <form method="post" id="userForm" autocomplete="off">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="text" id="new_username" name="new_username" required minlength="3" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                    </div>
                    <div>
                        <label for="new_password" class="block text-sm font-medium text-gray-700 mb-2">Password Baru</label>
                        <div class="relative">
                            <input type="password" id="new_password" name="new_password" required minlength="6" class="w-full px-4 py-3 pr-12 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                            <button type="button" class="js-toggle-password absolute inset-y-0 right-0 pr-4 flex items-center text-gray-400 hover:text-emerald-600 transition-colors" data-target="new_password" aria-label="Lihat password" title="Lihat Password">
                                <i class="fa-solid fa-eye" aria-hidden="true"></i>
                            </button>
                        </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="password" id="confirm_password" name="confirm_password" required minlength="6" class="w-full px-4 py-3 pr-12 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition">
                            <button type="button" class="js-toggle-password absolute inset-y-0 right-0 pr-4 flex items-center text-gray-400 hover:text-emerald-600 transition-colors" data-target="confirm_password" aria-label="Lihat password" title="Lihat Password">
                                <i class="fa-solid fa-eye" aria-hidden="true"></i>
                            </button>
                        </div>
                    </div>
                ...
                <div class="flex gap-3 mt-6">
                    <button type="submit" id="user_submit_btn" class="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-6 py-3 rounded-xl transition shadow-md shadow-emerald-200 flex items-center gap-2">
                        <i class="fa-solid fa-save" aria-hidden="true"></i>
                        <span id="user-submit-text">Tambah Pengguna</span>
                    </button>
                    <button type="button" id="cancel_edit_btn" class="bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold px-6 py-3 rounded-xl transition flex items-center gap-2 hidden">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 overflow-hidden">
            ...
            <div class="overflow-x-auto rounded-xl border border-gray-200">
                <table class="w-full text-sm text-left">
                    <thead class="bg-gray-50 text-gray-700 font-semibold border-b border-gray-200">
                        <tr><th class="p-4">No</th><th class="p-4">Username</th><th class="p-4">Role</th><th class="p-4">Dibuat Pada</th><th class="p-4 text-center">Aksi</th></tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="p-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button type="button" class="js-edit-user text-blue-600 hover:text-blue-800 hover:bg-blue-50 p-2 rounded-lg transition" data-id="<?= (string) $userId ?>" data-username="<?= admin_escape($username) ?>" title="Edit pengguna" aria-label="Edit pengguna <?= admin_escape($username) ?>">
                                        <i class="fa-solid fa-pen" aria-hidden="true"></i>
                                    </button>
                                    <?php if (!$isCurrentUser): ?>
                                    <a href="?hapus_user=<?= (string) $userId ?>" class="js-confirm text-red-600 hover:text-red-800 hover:bg-red-50 p-2 rounded-lg transition inline-flex items-center justify-center" data-message="Yakin hapus pengguna ini?" title="Hapus pengguna" aria-label="Hapus pengguna <?= admin_escape($username) ?>">
                                        <i class="fa-solid fa-trash" aria-hidden="true"></i>
                                    </a>
                                    <?php else: ?>
                                    <button type="button" class="text-gray-400 p-2 rounded-lg cursor-not-allowed" title="Tidak bisa hapus diri sendiri" aria-label="Tidak bisa hapus diri sendiri" disabled>
                                        <i class="fa-solid fa-trash" aria-hidden="true"></i>
                                    </button>
                                    <?php endif; ?>
                                </div>
...
<div id="mobile-sidebar" class="fixed inset-0 z-50 hidden md:hidden">
    ...
    <div class="absolute left-0 top-0 bottom-0 w-72 bg-white shadow-2xl transform transition-transform duration-300 -translate-x-full flex flex-col" id="mobile-sidebar-content">
        <div class="p-5 border-b border-gray-100 flex items-center justify-between bg-emerald-50">
            ...
            <button id="mobile-sidebar-close" class="text-gray-500 hover:text-red-600 hover:bg-red-50 p-2 rounded-lg transition-colors" aria-label="Tutup menu navigasi">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function openMobileSidebar() {
        if (!mobileSidebar || !mobileSidebarContent || !mobileSidebarBackdrop) { return; }
    ...
    function closeMobileSidebar() {
        ...
        setTimeout(function () { mobileSidebar.classList.add('hidden'); }, 300);
    }
    ...
    // ==================== SELECT "TAMPILKAN" (PER PAGE) ====================
    const perPageSelect = document.getElementById('per-page-select');
    if (perPageSelect) {
        perPageSelect.addEventListener('change', function () {
            const url = new URL(window.location.href);
            url.searchParams.set('tab', 'section-data-siswa');
    ...
    if (btnDeleteSelected) {
        btnDeleteSelected.addEventListener('click', function () {
            ...
            if (confirm('Hapus ' + checked.length + ' data yang dicentang?')) {
    ...
    if (btnDeleteAll) {
        btnDeleteAll.addEventListener('click', function () {
            const total = parseInt(btnDeleteAll.dataset.total || '0', 10);
            if (total === 0) {
                alert('Tidak ada data!');
                return;
            }
            if (confirm('Hapus SEMUA ' + total + ' data siswa (semua halaman)?') && confirm('Konfirmasi terakhir?')) {
    ...
    if (btnSaveSkl) {
        btnSaveSkl.addEventListener('click', function () {
            if (confirm('Simpan pengaturan Cetak SKL untuk data yang tampil di halaman ini?')) {
    ...
    function resetUserForm() {
        if (userAction) { userAction.value = 'tambah_pengguna'; }
        if (userId) { userId.value = ''; }
        if (userForm) { userForm.reset(); }
        if (newPassword) { newPassword.required = true; newPassword.value = ''; }
        if (confirmPassword) { confirmPassword.required = true; confirmPassword.value = ''; }
        if (passHint) { passHint.textContent = 'Wajib diisi untuk pengguna baru.'; }
        if (userSubmitText) { userSubmitText.textContent = 'Tambah Pengguna'; }
        if (userFormTitleText) { userFormTitleText.textContent = 'Tambah Pengguna Baru'; }
        if (userFormTitleIcon) { userFormTitleIcon.className = 'fa-solid fa-user-plus text-emerald-600 text-xl'; }
    ...
    document.querySelectorAll('.js-edit-user').forEach(function (button) {
        button.addEventListener('click', function () {
            ...
            if (userAction) { userAction.value = 'update_pengguna'; }
            if (userId) { userId.value = id; }
            if (newUsername) { newUsername.value = username; }
            if (newPassword) { newPassword.required = false; newPassword.value = ''; }
            if (confirmPassword) { confirmPassword.required = false; confirmPassword.value = ''; }
            if (passHint) { passHint.textContent = 'Kosongkan jika tidak ingin mengubah password.'; }
            if (userSubmitText) { userSubmitText.textContent = 'Perbarui Pengguna'; }
            if (userFormTitleText) { userFormTitleText.textContent = 'Edit Pengguna: ' + username; }
            if (userFormTitleIcon) { userFormTitleIcon.className = 'fa-solid fa-pen-to-square text-emerald-600 text-xl'; }
